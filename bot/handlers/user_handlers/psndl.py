...
async def func_psndl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game_name:
        return await effective_message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Website", PSNDL_WEBSITE_URL)]])
        )
    
    await effective_message.reply_text(
        f"Search result for <code>{game_name}</code>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
            f"Result('{game_name}')",
            f"{PSNDL_WEBSITE_URL}?name={quote(game_name)}"
        )]])
    )
