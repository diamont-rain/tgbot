...
async def func_genqr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        await effective_message.reply_text("Use <code>/genqr url/data/text</code> to generate a QR code.\nor reply the 'url/data/text' with <code>/genqr</code> command.\nE.g. <code>/genqr https://google.com</code>")
        return
