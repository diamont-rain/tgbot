...
async def func_kick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not re_msg:
        await effective_message.reply_text("I don't know who you are talking about! Reply the member whom you want to kick!\nE.g<code>/kick reason</code>")
        return
    
    if victim.id == context.bot.id:
        await effective_message.reply_text("I'm not going to kick myself!")
        return
    ...
    if chat_admins.is_victim_admin or chat_admins.is_victim_owner:
        await effective_message.reply_text("I'm not going to kick an admin! You must be kidding!")
        return
    ...
    if not is_silent:
        text = f"{victim.mention_html()} has been kicked out from this chat." + (f"\nReason: {reason}" if reason else "")
